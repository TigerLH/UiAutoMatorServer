...
import com.hcb.uiautomator.utils.ReflectionUtils;
...
public class AccessibilityNodeInfoDumper {
     ...
     * @param node
     */
    public static List<ElementInfo> dumpWindowToList(AccessibilityNodeInfo node){
    	indexMap.clear();
    	...
    	dumpNodeList(node, elements, width, height);
    	return elements;
   }
    
    
    private static void dumpNodeList(AccessibilityNodeInfo node,List<ElementInfo> elements,int width, int height){
//    	boolean tag = true;
//    	while(tag){
...
//    	}
    	...
    	int index;
    	if(indexMap.keySet().contains(node.getClassName().toString())){
    		int count = indexMap.get(node.getClassName().toString());
    		indexMap.put(node.getClassName().toString(), count+1);
    		index = count+1;
    	}else{
    		indexMap.put(node.getClassName().toString(), 0);
    		index = 0;
    	}
		 element.setIndex(index);
		 element.setText(safeCharSeqToString(node.getText()));
		 element.setPkg(safeCharSeqToString(node.getPackageName()));
		 element.setContentdesc(safeCharSeqToString(node.getContentDescription()));
		 element.setClassName(safeCharSeqToString(node.getClassName()));
	     ...
	     element.setSelected(node.isSelected());
  		 if (API.API_18){
  			element.setBounds(AccessibilityNodeInfoHelper.getVisibleBoundsInScreen(
  	              node, width, height));
  	        element.setResourceId(safeCharSeqToString(node.getViewIdResourceName()));
  		   }
         ...
         for (int i = 0; i < count; i++) {
             ...
             if (child != null) {
                 if (child.isVisibleToUser()) {
                	 dumpNodeList(child,elements, width, height);
                     child.recycle();
                 } else {
